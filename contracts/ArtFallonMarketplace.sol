...
interface IArtFallonAddressRegistry {
...
interface IArtFallonBundleMarketplace {
...
interface IArtFallonNFTFactory {
    function exists(address) external view returns (bool);
}

interface IArtFallonTokenRegistry {
    function enabled(address) external view returns (bool);
}

interface IArtFallonPriceFeed {
    function wFTM() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract ArtFallonMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IArtFallonAddressRegistry public addressRegistry;
    ...
    function _buyItem(
        ...
        IArtFallonBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createOffer(
        ...
        IArtFallonAuction auction = IArtFallonAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isArtFallonNFT(_nftAddress), "invalid nft address");
    ...
    function _isArtFallonNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.artion() == _nftAddress ||
            IArtFallonNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IArtFallonNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IArtFallonNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IArtFallonNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IArtFallonPriceFeed priceFeed = IArtFallonPriceFeed(
    ...
    /**
     @notice Update ArtFallonAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IArtFallonAddressRegistry(_registry);
    }
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IArtFallonTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
